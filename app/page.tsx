...
async function getDescription(
  videoId: string
): Promise<{ descriptionParagraph: string; resources: Resource[] }> {
...
async function getData() {
  const res = await fetch(
    `https://www.googleapis.com/youtube/v3/search?part=id,snippet&channelId=UCld68syR8Wi-GY_n4CaoJGA&maxResults=20&order=date&type=video&key=${process.env.API_KEY}`,
    {
      next: {
        revalidate: 60,
      },
  ...
  const videoDetailsPromises = data.items.map((item) =>
    getDescription(item.id.videoId)
  );
    ...
    <main className="md:max-w-3xl mx-auto">
      <div className="grid gap-4 px-4 py-8">
        ...
        {data.map((video) => (
          <div
            key={video.id}
            className="rounded-md bg-slate-700 border-slate-700 border p-4"
          >
            <p className="text-lg font-bold text-slate-300 pb-2">
              {video.title}
            </p>
            ...
            <ul>
              {video.resources.map((resource, index) => (
                <li
                  key={index}
                  className="before:content-['-'] before:absolute before:left-0 before:text-slate-500 relative pl-[1.2em]"
                >
                  <a href={resource.url} className="text-blue-400">
                    {resource.title}
                  </a>
                </li>
              ))}
